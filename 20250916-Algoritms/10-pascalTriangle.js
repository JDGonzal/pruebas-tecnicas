...
function mostrarTrianguloVisual(triangulo) {
  ...
  for (let i = 0; i < numFilas; i++) {
    ...
    const espaciosIniciales = ' '.repeat(
      (numFilas - i - 1) * (anchoMaximo + 1)
    );

    // Formatear cada número en la fila
    const filaFormateada = fila
      .map((num) => num.toString().padStart(anchoMaximo, ' '))
...
function validarTriangulo(triangulo) {
  ...
  for (let i = 0; i < triangulo.length; i++) {
    if (triangulo[i].length !== i + 1) {
      console.log(
        `❌ Error: La fila ${i} debería tener ${i + 1} elementos, pero tiene ${
          triangulo[i].length
        }`
      );
      esValido = false;
  ...
  for (let i = 0; i < Math.min(triangulo.length, 10); i++) {
    ...
    if (suma !== esperado) {
      console.log(
        `❌ Error: La suma de la fila ${i} es ${suma}, debería ser ${esperado}`
      );
      esValido = false;
    }
  }
...
function procesarTrianguloPascal() {
  rl.question(
    ...
    (respuesta) => {
      ...
      console.log(
        `📊 Total de elementos generados: ${triangulo.reduce(
          (acc, fila) => acc + fila.length,
          0
        )}`
      );
      ...
      console.log(
        '• Contiene secuencias famosas: números triangulares, Fibonacci, etc.'
      );
