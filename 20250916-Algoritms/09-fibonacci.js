    ...
    if (secuencia.length > 1) {
        ...
        if (secuencia.length >= 3) {
            ...
            if (secuencia.length >= 10) {
                const razonAurea = secuencia[ultimoIndice] / secuencia[ultimoIndice - 1];
                console.log(`Razón áurea aproximada: ${razonAurea.toFixed(6)} (φ ≈ 1.618034)`);
            }
        }
    }
}
...
function procesarFibonacci() {
    rl.question('Ingresa la cantidad de números de Fibonacci a generar: ', (respuesta) => {
        ...
        mostrarInformacionFibonacci(secuenciaIterativa, 'Iterativo', tiempoIterativo);
        ...
        if (cantidad <= 35) {
            ...
            mostrarInformacionFibonacci(secuenciaRecursiva, 'Recursivo', tiempoRecursivo);
            
            // Comparación de rendimiento
            console.log('\n⚡ Comparación de rendimiento:');
            console.log(`Método iterativo: ${tiempoIterativo.toFixed(4)} ms`);
            console.log(`Método recursivo: ${tiempoRecursivo.toFixed(4)} ms`);
            console.log(`Diferencia: ${(tiempoRecursivo / tiempoIterativo).toFixed(2)}x más lento el recursivo`);
        } else {
            console.log('\n⚠️  Método recursivo omitido para n > 35 (demasiado lento)');
            console.log('   La complejidad recursiva es O(2^n), mientras que la iterativa es O(n)');
        }
        ...
        console.log('• Aparece frecuentemente en la naturaleza (espirales, flores, etc.)');
        console.log('• La razón entre números consecutivos tiende a φ (phi) ≈ 1.618034');
